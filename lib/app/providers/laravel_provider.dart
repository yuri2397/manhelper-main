...
class LaravelApiClient extends GetxService with ApiClient {
  ...
  Future<bool> postRequest(String description, String categoryId,
    ...
    List<dio.MultipartFile> f = [];

    for (var item in files) {
      await f.add(await dio.MultipartFile.fromFile(item.path,
    ...
    Uri _uri = getApiBaseUri("post_request")
        .replace(queryParameters: _queryParameters);
  ...
  Future<List<PostRequest>> getPostRequests() async {
    if (!authService.isAuth) {
      throw new Exception(
          "You don't have the permission to access to this area!".tr +
              "[ List of post request ]");
    }
    List<PostRequest> res = [];
    var _queryParameters = {
      'api_token': authService.apiToken,
    };
    Uri _uri = getApiBaseUri("user_post_request")
        .replace(queryParameters: _queryParameters);

    var response;

    try {
      response = await _httpClient.getUri(_uri,
          options: dio.Options(
              followRedirects: false,
              validateStatus: (status) {
                return status < 500;
      ...
      print("USER POST REQUEST RESPONSE ${response}\n\n\n");
      try {
        res = postRequestFromJson(json.encode(response.data));
      } catch (e) {
        print("ERROR PARSE $e");
      }
    } catch (e) {
      print("ERROR GET POST REQUEST $e");
    } finally {
      return res;
    }
  }
